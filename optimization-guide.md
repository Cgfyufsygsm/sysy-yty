...
1) 接入 `-perf` 优化入口  
   - 在 `src/main.rs` 中，当模式为 `-perf` 时调用 `optimizer::optimize_ir`。  
   - 生成汇编后额外调用 `optimizer::optimize_backend`，预留后端优化入口。  
   - `src/optimizer/` 作为独立模块，包含 IR 与后端两个子模块。

2) 常量条件折叠 (IR 生成阶段)  
   - `if`：条件可折叠为常量时，仅生成命中的分支，避免创建多余基本块。  
   - `while`：`while(0)` 直接跳过循环体；`while(1)` 生成无条件回边的循环体，省掉条件判断。

3) IR 级 DCE（死代码消除）  
   - 保守地认为 `store/call/branch/jump/return` 有副作用，其依赖链保留，其余局部指令可删除。

4) 后端窥孔优化（第一步）  
   - 消除紧邻的 `sw` + `lw` 回读；若寄存器不同则用 `mv` 代替。
...
6) IR 优化模块拆分  
   - `src/optimizer/ir/` 目录下按 pass 拆分：`cfg.rs` 与 `dce.rs`，由 `mod.rs` 统一调度。

## 未来计划 (近期开工顺序)

1) 常量传播 + 再跑 DCE：把可传播的常量替换到使用处，清理由此产生的死代码。  
2) 后端窥孔优化（扩展）：跨少量无关指令的 load/store 简化。  
3) 控制流化简（完善）：尝试清理 DFG 中的不可达块与指令。
