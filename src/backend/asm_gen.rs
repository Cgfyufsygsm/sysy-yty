...
use crate::backend::{
    ...
    pipeline::AsmProgram,
    reg::RegGuard,
    util::{addi, calculate_size, load_operand_to_reg, lw, sw},
};
use koopa::ir::{
    entities::ValueData,
    values::{
...
pub trait GenerateProgAsm {
    fn generate_asm(&self) -> AsmProgram;
}
...
fn generate_global_alloc(program: &Program, value: &Value) -> String {
...
impl GenerateProgAsm for Program {
    fn generate_asm(&self) -> AsmProgram {
        ...
        for &value in self.inst_layout() {
            ...
            data.push_str(&format!("  .globl {}\n{}:\n", var_name, var_name));

            if let ValueKind::GlobalAlloc(global_alloc) = var_data.kind() {
                data.push_str(&generate_global_alloc(self, &global_alloc.init()));
            } else {
                unreachable!(
                    "Expected GlobalAlloc for global variable, found {:?}",
                    var_data.kind()
                );
            }
        }
        ...
        for &func in self.func_layout() {
            ...
            env.set_func(func);
            text.push_str(&func_data.generate_asm(&mut env));
            env.clear_func();
        }
        AsmProgram::from_sections(data, text)
    }
}
...
impl GenerateAsm for Binary {
    ...
    fn generate_asm(&self, env: &mut Environment) -> String {
        ...
        match self.op() {
            // 相等性比较
            Eq | NotEq => {
                let op_str = if matches!(self.op(), Eq) {
                    "seqz"
                } else {
                    "snez"
                };
