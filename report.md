...
本编译器最核心的数据结构是前端 AST、符号表与上下文，以及后端栈帧布局。前端使用 `SymbolTable` 维护多层作用域与全局函数/变量；`Context` 保存当前函数/基本块及循环栈信息；后端使用 `FrameLayout` 记录每个值在栈上的偏移与指针属性。
...
此外，为统一处理数组初始化，前端引入 `InitLike/InitIter` 以线性消费初始化项，支持嵌套大括号与扁平序列混用；后端通过 `ValueOffset { offset, is_ptr }` 标记 `getelemptr/getptr` 结果为指针，从而在 load/store 时采用不同访存策略。
...
#### 2.3.2 寄存器分配策略
未做全局寄存器分配，所有中间值统一落栈；临时计算使用寄存器池 `t0~t6`，通过 `RegGuard` 进行借用与归还。函数参数前 8 个使用 `a0~a7`，多余参数按 ABI 放入栈帧的 outgoing 区域；若函数内有调用则保存 `ra`。

#### 2.3.3 采用的优化策略
前端实现了常量折叠：`Fold/Eval` 在 AST 层将可计算表达式化简为常量；短路逻辑在 IR 中显式建块，避免无效求值。其余全局优化未实现。
